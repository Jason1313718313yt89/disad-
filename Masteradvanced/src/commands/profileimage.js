...
Jimp.read("https://cdn.discordapp.com/attachments/476516904736129026/483761118091935754/new_profiledddddddddddd.png", function (err, lenna) {
    Jimp.read(lenna.buffer, function (err, image) {
    // do stuff with the image (if no exception)
});
 
Jimp.read("https://cdn.discordapp.com/attachments/476516904736129026/483761118091935754/new_profiledddddddddddd.png", function (err, image) {
    Jimp.read(lenna.buffer, function (err, image) {
});
...
let user3 = user.id === "293805585378574336" 
  ...
  if (err) throw err;
    lenna.resize(1236, 729)            // resize
         .quality(100)            
        .print(font1, 340, 160, `${user.username}`)
                           .print(font1, 170, 350, user3? `${balance} Coins`:`${balance} Coins`)
                           .print(font1 , 170, 308 , warns[user.id].warns == 0? `0`:`${warns[user.id].warns}`)
                           .print(font1 , 170, 390 , `0`)
                           .print(font1 , 80, 510 , `${fetchedDB.text}`)
                            .composite( imagetouse, 57,97 )
